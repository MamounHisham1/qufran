        ...
        <main class="flex-grow md:p-5 md:w-2/3 md:m-0 mx-2 mb-2" x-data="home()">
            ...
            <div class="flex justify-around bg-gray-200 md:rounded-md p-3">
                <h2 x-on:click="active = 'quran'"
                    ...
                    :class="active === 'quran' ? 'bg-red-800 text-white' : 'bg-teal-800 text-gray-100 hover:bg-red-950 hover:text-white'">
                    {{ __('The Quran') }}
                </h2>
                <h2 x-on:click="active = 'hadith'"
                    class="text-lg font-semibold px-4 py-2 rounded-lg cursor-pointer select-none transition ease-in-out duration-300"
                    :class="active === 'hadith' ? 'bg-red-800 text-white' : 'bg-teal-800 text-gray-100 hover:bg-red-950 hover:text-white'">
                    {{ __('The Hadith') }}
                </h2>
                <h2 x-on:click="active = 'adhkar'"
                    class="text-lg font-semibold px-4 py-2 rounded-lg cursor-pointer select-none transition ease-in-out duration-300"
                    :class="active === 'adhkar' ? 'bg-red-800 text-white' : 'bg-teal-800 text-gray-100 hover:bg-red-950 hover:text-white'">
                    {{ __('The Adhkar') }}
                </h2>
            </div>
                    ...
                    @foreach ($books as $book)
                        <li>
                            <a href="{{ route('hadith.book', $book['collection'][0]['book']) }}"
                                ...
                                <span class="font-bold text-lg text-gray-800 group-hover:text-teal-600">{{ __($book['name']) }}</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
                    ...
                    @foreach ($adhkar as $key => $name)
                        <li>
                            <a href="{{ route('adhkar', $key + 1) }}"
                                class="block group px-4 py-3 transition bg-gray-50 hover:bg-teal-100 hover:shadow">
                                <span class="font-bold text-lg text-gray-800 group-hover:text-teal-600">{{ __($name) }}</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
